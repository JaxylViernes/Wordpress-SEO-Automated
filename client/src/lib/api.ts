...
export const api = {
  ...
  getWebsites: () =>
    fetch("/api/user/websites").then((res) => {
      if (!res.ok) throw new Error("Failed to fetch websites");
      return res.json();
    }),

  getWebsite: (id: string) =>
    fetch(`/api/user/websites/${id}`).then((res) => {
  ...
  getSeoHealth: () =>
    fetch("/api/seo/health").then((res) => {
      if (!res.ok) throw new Error("Failed to check SEO service health");
      return res.json();
    }),

  getAiProviderStatus: () =>
    fetch("/api/ai-providers/status").then((res) => {
  ...
  getDashboardStats: () =>
    fetch("/api/user/dashboard/stats").then((res) => {
      if (!res.ok) throw new Error("Failed to fetch dashboard stats");
      return res.json();
    }),

  getPerformanceData: () =>
    fetch("/api/user/dashboard/performance").then((res) => {
  ...
  getSeoTrends: async (websiteId: string, days: number = 30) => {
    ...
    const recentReports = reports.filter(
      (report) => new Date(report.createdAt) >= cutoffDate
    );
    ...
    return {
      ...
      averageScore:
        recentReports.length > 0
          ? Math.round(
              recentReports.reduce((sum, r) => sum + r.score, 0) /
  ...
  generateClientReport: (
    websiteId: string,
    data?: { reportType?: "weekly" | "monthly" | "quarterly" }
  ) => {
    ...
    return fetch(`/api/user/websites/${websiteId}/reports/generate`, {
      ...
      .then((res) => {
        ...
        if (!res.ok) {
          console.error(
            `Failed to generate report: ${res.status} ${res.statusText}`
          );
  ...
  getAvailableContentForScheduling: (websiteId: string) =>
    fetch(`/api/user/websites/${websiteId}/content`)
      .then((res) => {
