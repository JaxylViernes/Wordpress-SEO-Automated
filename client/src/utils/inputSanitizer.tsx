...
export class FrontendSanitizer {
  ...
  static validateSitemapUrl(url: string): {
    ...
    if (!hasTypicalPattern) {
      return {
        ...urlValidation,
        warning: 'URL does not follow typical sitemap naming (e.g., sitemap.xml)'
      };
    }
  ...
  static createRateLimiter(
    ...
    return (key: string): boolean => {
      ...
      const validRequests = keyRequests.filter(time => now - time < timeWindowMs);
...
export function useInputValidation() {
  ...
  const validateUrl = useCallback((value: string, fieldName: string = 'url') => {
    ...
    if (result.isValid) {
      ...
      setValidatedValues(prev => ({ ...prev, [fieldName]: result.sanitized }));
    } else {
      setErrors(prev => ({ ...prev, [fieldName]: result.error || 'Invalid URL' }));
      setWarnings(prev => {
        const newWarnings = { ...prev };
        delete newWarnings[fieldName];
        return newWarnings;
  ...
  const validateOAuth = useCallback((clientId: string, clientSecret: string) => {
    const result = FrontendSanitizer.validateOAuthCredentials(clientId, clientSecret);
  ...
  const validateBulkUrls = useCallback((urls: string) => {
    ...
    if (result.invalid.length > 0) {
      ...
      const errorMsg = result.invalid.length > 3
