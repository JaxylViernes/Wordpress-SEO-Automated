...
// Strip HTML tags from text to prevent them from appearing in metadata
...
export default function EnhancedImageMetadata() {
  ...
  const loadImages = async () => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        // Clean HTML from metadata fields when loading images
        const cleanedData = data.map(img => ({
          ...img,
          metadataDetails: img.metadataDetails ? {
            ...img.metadataDetails,
            caption: img.metadataDetails.caption 
              ? stripHtmlTags(img.metadataDetails.caption) 
              : img.metadataDetails.caption,
            altText: img.metadataDetails.altText 
              ? stripHtmlTags(img.metadataDetails.altText) 
              : img.metadataDetails.altText,
            description: img.metadataDetails.description 
              ? stripHtmlTags(img.metadataDetails.description) 
              : img.metadataDetails.description,
          } : img.metadataDetails
        }));
        
        // Combine with crawled images
        const allImages = [...cleanedData, ...crawledImages];
        setImages(allImages);
        
        if (cleanedData.length > 0) {
          const withMetadata = cleanedData.filter(img => img.hasMetadata).length;
          const withoutMetadata = cleanedData.length - withMetadata;
          
          addToast(
            `Loaded ${cleanedData.length} images (${withMetadata} protected, ${withoutMetadata} unprotected)`,
            "info"
          );
        }
      } else {
  ...
  const handleBatchProcess = async () => {
    ...
    try {
      ...
      // Clean HTML from metadata fields before sending to API
      const cleanedOptions = {
        ...processOptions,
        // Strip HTML from text fields that might contain it
        imageDescription: processOptions.imageDescription 
          ? stripHtmlTags(processOptions.imageDescription) 
          : processOptions.imageDescription,
        software: processOptions.software 
          ? stripHtmlTags(processOptions.software) 
          : processOptions.software,
        hostComputer: processOptions.hostComputer 
          ? stripHtmlTags(processOptions.hostComputer) 
          : processOptions.hostComputer,
        // These fields typically don't have HTML but clean them just in case
        copyright: processOptions.copyright 
          ? stripHtmlTags(processOptions.copyright) 
          : processOptions.copyright,
        author: processOptions.author 
          ? stripHtmlTags(processOptions.author) 
          : processOptions.author,
        make: processOptions.make 
          ? stripHtmlTags(processOptions.make) 
          : processOptions.make,
        model: processOptions.model 
          ? stripHtmlTags(processOptions.model) 
          : processOptions.model,
      };

      const result = await api.batchProcessMetadata(selectedImages, cleanedOptions);
