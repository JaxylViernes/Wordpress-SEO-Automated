...
export default function Websites() {
  ...
  const updateWebsiteMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => api.updateWebsite(id, data),
    onSuccess: () => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-lg font-medium text-gray-900 mb-2">Total Websites</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-lg font-medium text-gray-900 mb-2">Active Sites</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-lg font-medium text-gray-900 mb-2">Avg SEO Score</CardTitle>
              </CardHeader>
              <CardContent>
                  ...
                  <TableBody>
                    {filteredWebsites.map((website) => (
                      <TableRow key={website.id}>
                        ...
                        <TableCell>
                          <span className={`font-semibold ${getSeoScoreColor(website.seoScore)}`}>
                        ...
                        <TableCell>
                          <div className="flex items-center space-x-1">
                            ...
                            <span className="text-sm text-gray-500">
                              {format(new Date(website.updatedAt), "MMM dd, yyyy")}
                            </span>
                          </div>
                        </TableCell>
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedWebsite.name}</p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-gray-700">URL</Label>
                    <div className="mt-1 flex items-center space-x-2">
                      <p className="text-sm text-gray-900">{selectedWebsite.url}</p>
                      <a 
                  ...
                  <div>
                    ...
                    <p className={`mt-1 text-lg font-semibold ${getSeoScoreColor(selectedWebsite.seoScore)}`}>
                      {selectedWebsite.seoScore}/100
                    </p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-gray-700">Content Count</Label>
                    <p className="mt-1 text-sm text-gray-900">{selectedWebsite.contentCount} posts</p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-gray-700">AI Model</Label>
                    <p className="mt-1 text-sm text-gray-900">{selectedWebsite.aiModel || "Default"}</p>
                  </div>
                </div>
                ...
                {selectedWebsite.brandVoice && (
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedWebsite.brandVoice}</p>
                  </div>
                )}
                ...
                {selectedWebsite.targetAudience && (
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedWebsite.targetAudience}</p>
                  </div>
                )}
                
                {selectedWebsite.contentGuidelines && (
                  <div>
                    <Label className="text-sm font-medium text-gray-700">Content Guidelines</Label>
                    <p className="mt-1 text-sm text-gray-900 whitespace-pre-wrap">{selectedWebsite.contentGuidelines}</p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-4 pt-4 border-t">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">
                      {selectedWebsite.requireApproval !== false ? "Enabled" : "Disabled"}
                    </p>
                  </div>
                </div>
        ...
        <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
          <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditForm({...editForm, wpApplicationName: e.target.value})}
                    required
                  />
                </div>
              </div>
              ...
              <div>
                <Label htmlFor="wpApplicationPassword">Application Password *</Label>
                <Input
                  id="wpApplicationPassword"
                  type="password"
                  value={editForm.wpApplicationPassword}
                  onChange={(e) => setEditForm({...editForm, wpApplicationPassword: e.target.value})}
                  required
                />
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-2">
                  <Switch
                    ...
                    onCheckedChange={(checked) => setEditForm({...editForm, autoPosting: checked})}
                  />
        ...
        <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Are you sure you want to delete <strong>{websiteToDelete?.name}</strong>?
                ...
                <div className="mt-3 p-3 bg-red-50 rounded-md">
                  <p className="text-sm text-red-800">
                    <strong>Warning:</strong> This will NOT delete content from your actual WordPress site. 
                    Only the connection and data in this application will be removed.
                  </p>
                </div>
              </AlertDialogDescription>
            </AlertDialogHeader>
