...
export class CredentialRepairService {
   ...
   */
  async diagnoseCredentials(encryptedData: any, applicationName: string = 'AI Content Manager'): Promise<{
    canDecrypt: boolean;
    validFormat: boolean;
    issues: string[];
    ...
    if (issues.length > 0) {
      recommendations.push('Re-encrypt WordPress credentials with correct structure');
      return { canDecrypt: false, validFormat: false, issues, recommendations };
    }
    
    // Try to decrypt
    const decryptResult = this.authService.safeDecryptCredentials(encryptedData, applicationName);
    ...
    const passwordValidation = this.authService.validateApplicationPassword(credentials.applicationPassword);
    
    if (!passwordValidation.isValid) {
      issues.push(`Invalid password format: ${passwordValidation.issues.join(', ')}`);
      recommendations.push('Generate new WordPress Application Password');
      recommendations.push('Ensure password follows format: xxxx xxxx xxxx xxxx xxxx xxxx');
    }
    
    console.log(decryptResult.success ? '✅' : '❌', 'Decryption result:', decryptResult.success);
    console.log(passwordValidation.isValid ? '✅' : '❌', 'Password format valid:', passwordValidation.isValid);
  ...
  async repairCredentials(options: RepairOptions): Promise<{
    ...
    try {
      ...
      const validation = this.authService.validateApplicationPassword(options.newPassword);
      ...
      const roundtripTest = this.authService.testEncryptionRoundtrip(credentials);
  ...
  async testRepairedCredentials(
    ...
    try {
      ...
      const credentials = decryptResult.credentials!;
      const connectionTest = await this.authService.testConnectionWithDiagnostics(
        websiteUrl,
        credentials
      );
...
export async function repairWordPressCredentials(
  ...
  const diagnosis = await repairService.diagnoseCredentials(currentEncryptedData);
  ...
  if (!diagnosis.canDecrypt || !diagnosis.validFormat) {
    ...
    if (testResult.success) {
      ...
      console.error('❌ Repaired credentials still don\'t work:', testResult.error);
      console.log('Diagnostics:', testResult.diagnostics);
      return { success: false, error: testResult.error, diagnostics: testResult.diagnostics };
    }
  } else {
