...
export class ImageService {
  ...
  async generateImages(request: ImageGenerationRequest): Promise<ImageGenerationResult> {
    if (!process.env.OPENAI_API_KEY && !process.env.OPENAI_API_KEY_ENV_VAR) {
      throw new Error('OpenAI API key not configured - required for image generation');
    }

    console.log(`🎨 Starting image generation: ${request.count} images for "${request.topic}"`);
    ...
    for (let i = 0; i < request.count; i++) {
      try {
        const prompt = this.createImagePrompt(request.topic, request.style, i + 1, request.count, request.keywords);
        
        console.log(`🖼️ Generating image ${i + 1}/${request.count}: ${prompt.substring(0, 100)}...`);
        ...
        console.log(`✅ Generated image ${i + 1}/${request.count} (Cost: $${imageCost.toFixed(4)})`);
        ...
        if (error.status === 429) {
          throw new Error(`Rate limit exceeded during image generation. Generated ${i} of ${request.count} images. Please wait a minute and try again.`);
        } else if (error.status === 400 && error.message.includes('content_policy')) {
          console.warn(`⚠️ Content policy violation for image ${i + 1}, skipping...`);
          continue;
        } else if (error.status === 402) {
          throw new Error(`Insufficient OpenAI credits for image generation. Generated ${i} of ${request.count} images.`);
        }
        
        // For other errors, continue but log the failure
        console.warn(`⚠️ Skipping image ${i + 1} due to error: ${error.message}`);
      }
    }
    ...
    if (images.length < request.count) {
      console.warn(`⚠️ Generated ${images.length}/${request.count} images due to errors`);
    }

    console.log(`🎉 Image generation complete: ${images.length}/${request.count} images (Total cost: $${totalCost.toFixed(4)})`);
  ...
  async uploadImageToWordPress(
    ...
    filename: string, 
    altText: string, 
    wpCredentials: { url: string; username: string; applicationPassword: string }
    ...
    try {
      ...
      const authHeader = Buffer.from(`${wpCredentials.username}:${wpCredentials.applicationPassword}`).toString('base64');
  ...
  validateImageRequest(request: ImageGenerationRequest): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
