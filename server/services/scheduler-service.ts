...
export class SchedulerService {
  ...
  async processScheduledContent(): Promise<{
    ...
    try {
      ...
      console.log(`📋 Found ${overdueContent.length} scheduled items to process`);
      const results = [];
      ...
      for (const schedule of overdueContent) {
        try {
          ...
          if (!content) {
            ...
            results.push({ scheduleId: schedule.id, success: false, error: 'Content not found' });
            continue;
          }
          ...
          if (!website) {
            ...
            results.push({ scheduleId: schedule.id, success: false, error: 'Website not found' });
            continue;
          }
          
          // Publish the content
          const publishResult = await this.publishContentToWordPress(content, website);
          ...
          if (publishResult.success) {
            ...
            console.error(`❌ Failed to publish: ${content.title} - ${publishResult.error}`);
          }
          
        } catch (error) {
          console.error(`❌ Error processing schedule ${schedule.id}:`, error);
          await this.markScheduleFailed(schedule.id, error instanceof Error ? error.message : 'Unknown error');
          results.push({
            scheduleId: schedule.id,
            success: false,
      ...
      console.log(`🎯 Processing complete: ${published} published, ${failed} failed`);
  ...
  private async publishContentToWordPress(content: any, website: any): Promise<{
    success: boolean;
    postId?: number;
    url?: string;
    error?: string;
  ...
  startScheduler(intervalMinutes: number = 5): void {
    console.log(`🔄 Starting content scheduler (checking every ${intervalMinutes} minutes)`);
