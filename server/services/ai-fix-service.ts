...
class AIFixService {
  ...
  // Enhanced AI Provider Selection - Prioritizes Claude
  ...
  // Enhanced AI Provider Call with Claude support
  ...
  // Verify WordPress connection and permissions
  private async verifyWordPressConnection(creds: WordPressCredentials): Promise<{
    success: boolean;
    permissions: string[];
    error?: string;
  }> {
    try {
      const authTest = await fetch(`${creds.url}/wp-json/wp/v2/users/me`, {
        headers: {
      ...
      if (!authTest.ok) {
        return { 
          success: false, 
          permissions: [], 
          error: `Authentication failed: ${authTest.status}` 
        };
      }

      const user = await authTest.json();
      const capabilities = user.capabilities || {};
      const canEdit = capabilities.edit_posts && capabilities.edit_pages;
      
      return {
        success: canEdit,
        permissions: Object.keys(capabilities),
        error: canEdit ? undefined : 'User lacks edit permissions'
      };
    } catch (error) {
      return { 
        success: false, 
  ...
  async analyzeAndFixWebsite(
    ...
    try {
      ...
      // Get tracked issues that need fixing
      ...
      const fixableIssues = trackedIssues.map(trackedIssue => ({
        ...
        impact: this.mapSeverityToImpact(trackedIssue.severity),
      ...
      if (!dryRun && fixesToApply.length > 0) {
        ...
        // Apply fixes
        const applyResult = await this.applyComprehensiveFixes(website, fixesToApply);
        ...
        if (options.enableReanalysis !== false && (successfulFixes.length > 0 || options.forceReanalysis)) {
        ...
        if (options.enableReanalysis !== false && fixesToApply.length > 0) {
          ...
          reanalysisData = {
            ...
            finalScore: Math.min(100, latestReport.score + estimatedImprovement),
      ...
      const stats = this.calculateStats(appliedFixes, fixableIssues.length);
      const message = this.generateResultMessage(dryRun, stats, reanalysisData);
      ...
      this.addLog(`AI fix service error: ${error.message}`, "error");
      return {
        success: false,
        dryRun,
        fixesApplied: [],
  ...
  // Apply comprehensive fixes to WordPress
  private async applyComprehensiveFixes(
    ...
    if (!creds.applicationPassword) {
      throw new Error("Missing WordPress application password");
    }

    // Verify connection
    const connectionTest = await this.verifyWordPressConnection(creds);
    if (!connectionTest.success) {
      throw new Error(connectionTest.error || "WordPress connection failed");
    }
    ...
    for (const [fixType, typeFixes] of Object.entries(fixesByType)) {
      ...
      try {
        ...
        for (const fix of result.applied) {
          if (fix.success && fix.wordpressPostId) {
            const verified = await this.verifyFixWasApplied(
              creds,
              fix,
              fix.wordpressPostId,
              'post' // Default to post, could be enhanced
            );
            fix.verified = verified;
            if (!verified) {
              this.addLog(`Fix verification failed for ${fix.type}`, "warning");
            }
          }
        }

        const successCount = result.applied.filter((f) => f.success && f.verified !== false).length;
        this.addLog(
          `Completed ${fixType}: ${successCount}/${result.applied.length} successful and verified`,
          successCount > 0 ? "success" : "warning"
        );
      } catch (error) {
        const errorMessage = `Failed to apply ${fixType} fixes: ${error.message}`;
  ...
  private async applyFixesByType(
    ...
    switch (fixType) {
      case "missing_alt_text":
        return this.fixImageAltText(creds, fixes);
      case "missing_meta_description":
        return this.fixMetaDescriptions(creds, fixes);
      case "poor_title_tag":
        return this.fixTitleTags(creds, fixes);
      case "heading_structure":
      case "missing_h1_tag":
      case "missing_h1":
        return this.fixHeadingStructure(creds, fixes);
      case "content_quality":
      case "low_content_quality":
        return this.fixContentQuality(creds, fixes);
      case "keyword_optimization":
      case "poor_keyword_distribution":
        return this.fixKeywordOptimization(creds, fixes);
      case "missing_viewport_meta":
        return this.fixViewportMetaTag(creds, fixes);
      case "missing_schema":
        return this.fixSchemaMarkup(creds, fixes);
      case "missing_og_tags":
        return this.fixOpenGraphTags(creds, fixes);
      default:
        ...
        return {
          ...
          errors: [`Fix type '${fixType}' not implemented yet`],
        };
    }
  }
  ...
  private async fixImageAltText(
    ...
    try {
      const allContent = await this.getAllWordPressContent(creds);
          ...
          if (imagesWithoutAlt.length > 0) {
            ...
            for (let i = 0; i < Math.min(imagesWithoutAlt.length, 5); i++) {
              ...
              if (src && !src.startsWith("data:")) {
                const altText = await this.generateAltText(src, content.title.rendered);
                ...
                applied.push({
                  ...
                  description: `Added alt text to image in "${content.title.rendered}"`,
            ...
            if (contentModified) {
              ...
              this.addLog(`Updated content for: ${content.title.rendered}`, "success");
            }
          }
        } catch (contentError) {
          errors.push(`Error processing content: ${contentError.message}`);
          this.addLog(contentError.message, "warning");
        }
      }

      this.addLog(`Fixed alt text for ${totalImagesFixed} images total`, "success");
    } catch (error) {
      errors.push(`Alt text fix failed: ${error.message}`);
    ...
    try {
        ...
        try {
          ...
          if (!excerpt || excerpt.length < 120 || excerpt.length > 160) {
            ...
            applied.push({
              ...
              description: `Updated meta description for "${content.title.rendered}"`,
            ...
            fixedCount++;
            this.addLog(`Updated meta description for: ${content.title.rendered}`, "success");
          } else {
            alreadyAdequateCount++;
            this.addLog(`Meta description already adequate for "${content.title.rendered}"`, "info");
          }
        } catch (contentError) {
          this.addLog(`Error processing meta description: ${contentError.message}`, "warning");
        }
      }
      ...
      if (processedCount > 0 && alreadyAdequateCount === processedCount && fixedCount === 0) {
        applied.push({
          ...
          after: `All adequate (120-160 chars)`,
          success: true,
          impact: "low",
      ...
      errors.push(`Meta description fix failed: ${error.message}`);
      this.addLog(error.message, "error");
    }
        ...
        try {
          ...
          if (currentTitle.length > 60 || currentTitle.length < 30) {
            const optimizedTitle = await this.optimizeTitle(
              currentTitle,
              this.extractTextFromHTML(content.content?.rendered || "")
            );
          ...
          this.addLog(`Error processing title: ${contentError.message}`, "warning");
        }
      }
    } catch (error) {
      errors.push(`Title optimization failed: ${error.message}`);
      this.addLog(error.message, "error");
    }
    ...
    try {
      ...
      for (const content of allContent.slice(0, 10)) {
        try {
          ...
          // Add H1 if missing
          if (h1s.length === 0) {
            const title = content.title?.rendered || "Page Title";
          ...
          if (needsUpdate) {
            ...
            fixedCount++;
            this.addLog(`Fixed heading structure for: ${content.title.rendered}`, "success");
          } else {
            alreadyCorrectCount++;
            this.addLog(`Heading structure already correct for "${content.title.rendered}"`, "info");
          }
        } catch (contentError) {
          this.addLog(`Error processing headings: ${contentError.message}`, "warning");
        }
      }
      ...
      if (processedCount > 0 && (alreadyCorrectCount > 0 || fixedCount > 0)) {
        applied.push({
          ...
          before: `Checked ${processedCount} pages`,
          after: `${alreadyCorrectCount} correct, ${fixedCount} fixed`,
          success: true,
      ...
      errors.push(`Heading structure fix failed: ${error.message}`);
      this.addLog(error.message, "error");
    }
  ...
  private async fixContentQuality(
    ...
    try {
      ...
      for (const content of allContent.slice(0, 5)) {
        try {
          ...
          if (analysis.score < 75 && analysis.improvements.length > 0) {
            ...
            if (improvedContent && improvedContent !== content.content.rendered) {
              await this.updateWordPressContent(creds, content.id, {
              ...
              applied.push({
                ...
                after: `Improved readability and engagement`,
                success: true,
                impact: "high",
                wordpressPostId: content.id,
              });

              this.addLog(`Improved content quality for: "${title}"`, "success");
            }
          }
        } catch (contentError) {
          this.addLog(`Error analyzing content: ${contentError.message}`, "warning");
        }
      }
    } catch (error) {
      errors.push(`Content quality improvement failed: ${error.message}`);
      this.addLog(error.message, "error");
    }
          ...
          if (keywordAnalysis.needsOptimization) {
            ...
            if (optimizedContent !== contentHtml) {
              ...
              applied.push({
                ...
                description: `Improved keyword distribution`,
                element: `${content.contentType}-${content.id}`,
  ...
  private async fixViewportMetaTag(
    ...
    // Viewport typically requires theme modification
    return {
      applied: fixes.map(fix => ({
        ...fix,
        success: false,
        error: "Viewport meta tag requires theme modification"
      })),
      errors: ["Viewport meta tag requires theme modification"]
    };
  }
  ...
  private async fixSchemaMarkup(
    ...
    const applied: AIFix[] = [];
    const errors: string[] = [];
    // Implementation would add JSON-LD schema to content
    return { applied, errors };
  }
  ...
  private async fixOpenGraphTags(
    ...
    const applied: AIFix[] = [];
    const errors: string[] = [];
    // Implementation would add OG meta tags
    return { applied, errors };
  }

  // Get all WordPress content
  private async getAllWordPressContent(creds: WordPressCredentials): Promise<WordPressContent[]> {
    const [posts, pages] = await Promise.all([
      this.getWordPressContentWithType(creds, "posts"),
      this.getWordPressContentWithType(creds, "pages"),
    ]);
    return [...posts, ...pages];
  }

  // Get WordPress content with type
  private async getWordPressContentWithType(
    ...
    try {
      ...
      console.error(`Failed to fetch WordPress ${type}:`, error);
      return [];
    }
  }
  ...
  private async updateWordPressContent(
    ...
    const endpoint = `${creds.url.replace(/\/$/, "")}/wp-json/wp/v2/${contentType}s/${id}`;
    const auth = Buffer.from(
      `${creds.username}:${creds.applicationPassword}`
    ).toString("base64");

    console.log(`Updating WordPress ${contentType} ${id}`);
    ...
    if (!response.ok) {
      ...
      throw new Error(`WordPress update failed: ${response.status} - ${errorBody}`);
    }
  ...
  private async verifyFixWasApplied(
    creds: WordPressCredentials,
    fix: AIFix,
    contentId: number,
    contentType: string
    ...
    try {
      ...
      const endpoint = `${creds.url}/wp-json/wp/v2/${contentType}s/${contentId}`;
      const response = await fetch(endpoint);
      ...
      switch (fix.type) {
        case 'missing_meta_description':
          return content.excerpt?.rendered?.length > 0;
        case 'missing_h1_tag':
        case 'heading_structure':
          return content.content?.rendered?.includes('<h1');
        case 'missing_alt_text':
          const imgCount = (content.content?.rendered?.match(/<img[^>]+alt="/g) || []).length;
          return imgCount > 0;
        default:
          return content.modified !== content.date;
      }
    } catch (error) {
  ...
  // Update issue statuses based on fixes
  private async updateIssueStatusesForFix(
    ...
    for (const fix of fixes) {
      const matchingIssue = trackedIssues.find(i => 
        i.id === fix.trackedIssueId || 
        this.issueMatchesFix(i, fix)
      );
      ...
      if (matchingIssue && !processedIssueIds.has(matchingIssue.id)) {
        ...
        if (fix.success) {
          const actuallyFixed = this.wasActualFixApplied(fix);
          
          if (actuallyFixed) {
            newStatus = 'fixed';
          } else if (this.wasVerifiedAsAlreadyGood(fix)) {
            console.log(`Issue ${matchingIssue.id} verified as already adequate - no status change`);
        ...
        await storage.updateSeoIssueStatus(matchingIssue.id, newStatus, {
          ...
          fixSessionId,
          actuallyFixed: newStatus === 'fixed',
          resolutionNotes: this.generateResolutionNote(fix, newStatus)
        });
    ...
    // Reset unprocessed fixing issues
  ...
  // Check if issue matches fix
  private issueMatchesFix(issue: any, fix: AIFix): boolean {
    const titleLower = issue.issueTitle.toLowerCase();
    const fixType = fix.type.toLowerCase();
    
    if (titleLower.includes(fixType)) return true;
    if (issue.issueType === fix.type) return true;
  ...
  private wasActualFixApplied(fix: AIFix): boolean {
    ...
    if (verificationPhrases.some(phrase => description.includes(phrase))) {
      return false;
    }
    ...
    return fixPhrases.some(phrase => description.includes(phrase));
  }

  // Check if issue was verified as already good
  private wasVerifiedAsAlreadyGood(fix: AIFix): boolean {
    const description = fix.description.toLowerCase();
    return description.includes('already adequate') || 
           description.includes('already correct') ||
           description.includes('verified');
  }

  // Generate resolution note
  private generateResolutionNote(fix: AIFix, status: string): string {
    if (fix.success && status === 'fixed') {
      return `Issue fixed: ${fix.description}`;
    } else if (fix.success && status === 'resolved') {
      return `Issue was already resolved: ${fix.description}`;
    } else if (!fix.success) {
      return `Fix failed: ${fix.error || 'Unknown error'}`;
    }
    return `Fix attempted: ${fix.description}`;
  }
  ...
  private async performReanalysis(
    ...
    while (attempt < maxRetries) {
      ...
      try {
        this.addLog(`Reanalysis attempt ${attempt} of ${maxRetries}...`, "info");
        ...
        if (!newAnalysis || typeof newAnalysis.score !== 'number') {
        ...
        this.addLog(`Reanalysis attempt ${attempt} failed: ${error.message}`, "warning");
        
        if (attempt < maxRetries) {
          const retryDelay = Math.min(delay * Math.pow(2, attempt - 1), 30000);
    ...
    return {
      ...
      scoreImprovement: 0,
      success: false,
      error: lastError?.message || 'Reanalysis failed',
    };
  }
  ...
  private async analyzeContentQuality(content: string, title: string): Promise<{
    ...
    try {
      const systemPrompt = `Analyze content quality and return JSON with score, issues, improvements, and readabilityScore.`;
  ...
  private async improveContentQuality(
    ...
    try {
      const systemPrompt = `Improve content based on suggestions. Return only improved HTML content.`;
      const userPrompt = `Title: "${title}"\nContent: ${content}\nImprovements: ${improvements.join(', ')}`;

      return await this.callAIProvider(provider, systemPrompt, userPrompt, 2000, 0.4);
    } catch (error) {
      return content;
    }
  }
  ...
  private async analyzeKeywordUsage(
    ...
    try {
      const systemPrompt = `Analyze keyword usage and return JSON with optimization needs.`;
  ...
  private async optimizeKeywordDistribution(
    ...
    try {
      const systemPrompt = `Optimize keyword distribution naturally. Return optimized HTML content.`;
      const userPrompt = `Keywords: ${targetKeywords.join(", ")}\nMissing: ${missingKeywords.join(", ")}\nContent: ${contentHtml.substring(0, 3000)}`;

      return await this.callAIProvider(provider, systemPrompt, userPrompt, 3000, 0.4);
    } catch (error) {
      return contentHtml;
    }
  }
  ...
  private async generateAltText(imageSrc: string, context: string): Promise<string> {
    ...
    if (!provider) {
      ...
      return filename.replace(/[-_]/g, " ").substring(0, 100) || "Image";
    }

    try {
      const prompt = `Generate descriptive alt text for image "${imageSrc}" in context "${context}". Max 100 characters.`;
      ...
      return result.trim().replace(/^["']|["']$/g, "").substring(0, 100);
    } catch (error) {
      const filename = imageSrc.split("/").pop()?.replace(/\.[^/.]+$/, "") || "";
      return filename.replace(/[-_]/g, " ").substring(0, 100) || "Image";
    }
  ...
  private async generateMetaDescription(title: string, content: string): Promise<string> {
    ...
    try {
      const prompt = `Meta description for "${title}". Content: "${content.substring(0, 300)}". 120-160 chars.`;
      const result = await this.callAIProvider(
        provider,
  ...
  private async optimizeTitle(currentTitle: string, content: string): Promise<string> {
    ...
    try {
      const prompt = `Optimize title "${currentTitle}" using content "${content.substring(0, 200)}". 30-60 chars.`;
      const result = await this.callAIProvider(
        provider,
  ...
  private estimateScoreImprovement(fixes: AIFix[]): number {
    ...
    fixes.forEach(fix => {
      if (fix.success && this.wasActualFixApplied(fix)) {
        const count = actuallyFixedCount.get(fix.type) || 0;
        actuallyFixedCount.set(fix.type, count + 1);
      }
    });
    ...
    actuallyFixedCount.forEach((count, fixType) => {
      ...
      estimatedImprovement += typeScore;
    });
    
    return Math.min(40, Math.round(estimatedImprovement * 10) / 10);
  }
  ...
  private calculateStats(fixes: AIFix[], totalIssues: number) {
    const successful = fixes.filter((f) => f.success && f.verified !== false);
    
    return {
      totalIssuesFound: totalIssues,
      fixesAttempted: fixes.length,
      fixesSuccessful: successful.length,
      fixesFailed: fixes.length - successful.length,
      estimatedImpact: this.calculateEstimatedImpact(successful),
      detailedBreakdown: {
  ...
  private calculateEstimatedImpact(fixes: AIFix[]): string {
    const highImpactFixes = fixes.filter((f) => f.impact === "high").length;
    const mediumImpactFixes = fixes.filter((f) => f.impact === "medium").length;
    ...
    if (mediumImpactFixes >= 1) return "low";
    return "minimal";
  }

  private generateResultMessage(dryRun: boolean, stats: any, reanalysis: any): string {
    let message = dryRun
    ...
    if (reanalysis && reanalysis.success) {
      if (reanalysis.simulated) {
        message += ` Estimated SEO score improvement: +${reanalysis.scoreImprovement} points`;
      } else {
  ...
  private async createWebsiteBackup(website: any, userId: string): Promise<void> {
    ...
    try {
      ...
      this.addLog(`Backup creation failed: ${error.message}`, "warning");
    }
  }
