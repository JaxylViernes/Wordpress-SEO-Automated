...
class AIFixService {
  ...
  private addLog(message: string, level: 'info' | 'success' | 'warning' | 'error' = 'info') {
  ...
  async analyzeAndFixWebsite(
    ...
    try {
      this.addLog(`Starting AI fix analysis for website ${websiteId} (dry run: ${dryRun})`);
      ...
      const analysisResult = await this.analyzeWebsiteForAllFixes(website.url, latestReport);
      
      this.addLog(`Analysis found ${analysisResult.totalIssues} total issues`);
      this.addLog(`Fixable issues identified: ${analysisResult.fixableIssues.length}`);

      const maxChanges = options.maxChanges || analysisResult.fixableIssues.length;
      ...
      if (!dryRun && fixesToApply.length > 0) {
        ...
        this.addLog(`Applied ${successfulFixes.length}/${appliedFixes.length} fixes successfully`, 'success');
      } else {
        ...
        this.addLog(`Dry run complete - would apply ${appliedFixes.length} fixes`, 'info');
      }
      ...
      this.addLog(`Final stats: ${stats.fixesSuccessful}/${stats.fixesAttempted} fixes successful`, 
      ...
      this.addLog(`AI fix service error: ${error instanceof Error ? error.message : 'Unknown error'}`, 'error');
  ...
  private async analyzeWebsiteForAllFixes(url: string, seoReport: any) {
    ...
    try {
      ...
      const websiteContent = await this.fetchWebsiteContentWithTimeout(url, 10000); // 10 second timeout
      ...
      this.addLog(`Direct HTML analysis found: ${directAnalysis.fixes.length} issues`);

      // Try AI analysis with enhanced error handling
      let aiAnalysis = { fixes: [], totalIssues: 0, recommendations: [] };
      try {
        aiAnalysis = await this.getAIFixRecommendations(url, websiteContent, seoReport);
        this.addLog(`AI analysis found: ${aiAnalysis.fixes.length} issues`);
      } catch (aiError) {
        this.addLog(`AI analysis failed, using HTML analysis only: ${aiError instanceof Error ? aiError.message : 'Unknown error'}`, 'warning');
      }
      ...
      this.addLog(`Total unique issues after deduplication: ${deduplicatedFixes.length}`);

      return {
        totalIssues: directAnalysis.totalIssues + aiAnalysis.totalIssues,
        fixableIssues: deduplicatedFixes,
        recommendations: [...directAnalysis.recommendations, ...aiAnalysis.recommendations]
      };
    } catch (error) {
      this.addLog(`Website analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'error');
  ...
  private async getAIFixRecommendations(url: string, content: string, seoReport: any) {
...
Find the top 5 most important automated fixes. Return only JSON.`;
    ...
    try {
      ...
      try {
        ...
        this.addLog(`Initial JSON parse failed, attempting to fix...`, 'warning');
      ...
      this.addLog(`AI analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'error');
      throw error; // Re-throw to be handled by caller
    }
  }

  // Enhanced JSON fixing with multiple strategies
  private tryFixMalformedJSONMultipleAttempts(jsonString: string): string | null {
    const strategies = [
  ...
  this.log = []; // Reset log
  this.addLog(`Starting iterative AI fix process (target: ${targetScore}, max iterations: ${maxIterations})`);
  ...
  try {
    ...
    this.addLog(`Starting iterative fixes for website: ${website.name} (${website.url})`);

    // Get initial SEO score
    let currentScore = await this.getCurrentSeoScore(websiteId);
    if (currentScore === null) {
      this.addLog('No existing SEO analysis found. Running initial analysis...', 'info');
      const initialAnalysis = await seoService.analyzeWebsite(website.url, []);
    ...
    if (currentScore >= targetScore) {
      this.addLog(`Website already meets target score (${currentScore} >= ${targetScore})`, 'success');
      stoppedReason = 'target_reached';
    } else {
      // Create backup before starting (unless skipped)
      if (!skipBackup) {
        await this.createWebsiteBackup(website, userId);
        this.addLog('Website backup created before iterative fixes', 'success');
      }
      ...
      while (currentIteration < maxIterations && currentScore < targetScore) {
        ...
        if (!fixResult.success) {
          this.addLog(`Iteration ${currentIteration} failed: ${fixResult.message}`, 'error');
          allErrors.push(`Iteration ${currentIteration}: ${fixResult.message}`);
          if (fixResult.errors) {
        ...
        this.addLog(`Applied ${fixResult.stats.fixesSuccessful} fixes in iteration ${currentIteration}`);
        ...
        this.addLog(`Score changed: ${currentScore} → ${newScore} (${improvement >= 0 ? '+' : ''}${improvement.toFixed(1)})`, 
          improvement > 0 ? 'success' : 'warning');

        // Check if we've reached the target
        if (newScore >= targetScore) {
          this.addLog(`Target score reached! ${newScore} >= ${targetScore}`, 'success');
          stoppedReason = 'target_reached';
        ...
        if (improvement < minImprovementThreshold) {
          this.addLog(`Improvement too small (${improvement.toFixed(1)} < ${minImprovementThreshold}). Stopping.`, 'warning');
      ...
      if (currentIteration >= maxIterations && currentScore < targetScore) {
        this.addLog(`Reached maximum iterations (${maxIterations}) without achieving target score`, 'warning');
    ...
    await storage.createActivityLog({
      ...
      description: `Iterative AI fixes completed: ${initialScore} → ${finalScore} (+${scoreImprovement.toFixed(1)}) in ${currentIteration} iterations`,
      metadata: {
        initialScore,
        finalScore,
        scoreImprovement,
    ...
    this.addLog(`Improvement: ${scoreImprovement >= 0 ? '+' : ''}${scoreImprovement.toFixed(1)} points`);
    this.addLog(`Iterations completed: ${currentIteration}`);
    this.addLog(`Reason for stopping: ${stoppedReason}`);
    this.addLog(`Total fixes applied: ${allAppliedFixes.filter(f => f.success).length}`);
    ...
    return {
      ...
      message: `Iterative AI fixes completed: ${initialScore} → ${finalScore} (+${scoreImprovement.toFixed(1)}) in ${currentIteration} iterations`,
      detailedLog: [...this.log],
    ...
    this.addLog(`Iterative fix process failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'error');
    ...
    return {
      ...
      message: `Iterative AI fix failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      detailedLog: [...this.log],
      // Extended iterative properties
      iterations,
...
private async getCurrentSeoScore(websiteId: string): Promise<number | null> {
  try {
    ...
    this.addLog(`Failed to get current SEO score: ${error instanceof Error ? error.message : 'Unknown error'}`, 'warning');
    return null;
  }
}
...
static estimateIterationsNeeded(currentScore: number, targetScore: number, averageImprovementPerIteration: number = 5): number {
  ...
  private validateAnalysisStructure(analysis: any): boolean {
    return (
      ...
      analysis.fixes.every((fix: any) => 
        fix.type && fix.description && fix.impact
      )
    );
  }
  ...
  private createFallbackAnalysis(seoReport: any) {
    ...
    return {
      ...
      recommendations: ['Run a fresh SEO analysis for detailed recommendations']
    };
  }

  private async fetchWebsiteContentWithTimeout(url: string, timeout: number = 10000): Promise<string> {
  ...
  private analyzeHTMLDirectly($: cheerio.CheerioAPI) {
    ...
    if (metaDesc.length === 0) {
      ...
      if (content.length < 120 || content.length > 160) {
        fixes.push({
          type: 'missing_meta_description',
          description: `Meta description is ${content.length < 120 ? 'too short' : 'too long'} (${content.length} chars)`,
          element: 'meta[name="description"]',
    ...
    if (title.length === 0 || !title.text().trim()) {
      ...
      fixes.push({
        type: 'poor_title_tag',
        description: `Title tag is ${title.text().length > 60 ? 'too long' : 'too short'} (${title.text().length} chars)`,
        element: 'title',
  ...
  private async applyComprehensiveFixes(
    ...
    this.addLog(`Applying ${fixes.length} comprehensive fixes to WordPress site: ${website.url}`);
    ...
    if (!creds.applicationPassword) {
      throw new Error('Missing WordPress application password. Please configure WordPress credentials.');
    }

    this.addLog(`WordPress connection: ${creds.url}, User: ${creds.username}, Password: ${creds.applicationPassword}`, 'info');
  ...
  this.addLog(`Connection test failed: ${connectionError instanceof Error ? connectionError.message : 'Unknown error'}`, 'error');
  throw connectionError;
    ...
    for (const [fixType, typeFixes] of Object.entries(fixesByType)) {
      ...
      try {
        ...
        this.addLog(`Completed ${fixType}: ${successCount}/${result.applied.length} successful`, 
                     successCount > 0 ? 'success' : 'warning');
      } catch (error) {
        const errorMessage = `Failed to apply ${fixType} fixes: ${error instanceof Error ? error.message : 'Unknown error'}`;
  ...
  private async testWordPressConnectionEnhanced(creds: WordPressCredentials) {
    const diagnostics = {
    ...
    try {
      ...
      const apiResponse = await fetch(`${creds.url}/wp-json/wp/v2/users/me`, {
        method: 'GET',
        headers: {
      ...
      diagnostics.restApiAvailable = true;

      if (apiResponse.ok) {
        diagnostics.authenticationWorking = true;
        diagnostics.userInfo = await apiResponse.json();
        return { success: true, diagnostics };
      } else {
        const errorBody = await apiResponse.text();
        
        if (apiResponse.status === 401) {
          diagnostics.errors.push('Authentication failed - Invalid username or Application Password');
          diagnostics.recommendations.push(
            'Verify the WordPress username is correct',
          ...
          diagnostics.errors.push('Access forbidden - User may not have sufficient permissions');
          diagnostics.recommendations.push(
            'Ensure the user has administrator or editor privileges',
            'Check if security plugins are blocking REST API access',
            'Verify Application Passwords are enabled in WordPress'
          );
        } else {
          diagnostics.errors.push(`HTTP ${apiResponse.status}: ${errorBody}`);
        }
        ...
        if (responseHeaders['server']?.includes('LiteSpeed')) {
          diagnostics.recommendations.push('LiteSpeed server detected - check LiteSpeed cache settings for REST API exclusions');
        }
        if (responseHeaders['panel'] === 'hpanel') {
          diagnostics.recommendations.push('Hostinger hosting detected - check if they have security features blocking external API access');
        }
      ...
      diagnostics.recommendations.push(
        ...
        'Verify WordPress is installed and REST API is enabled',
        'Check firewall settings that might block API requests'
      );
  ...
  private generateTroubleshootingMessage(connectionTest: any): string {
    const lines = [
      'WordPress connection failed',
      '',
      'Issues found:',
      ...connectionTest.diagnostics.errors.map((error: string) => `• ${error}`),
      '',
      'Troubleshooting steps:',
      ...connectionTest.diagnostics.recommendations.map((rec: string, index: number) => `${index + 1}. ${rec}`),
      '',
      'Common solutions:',
      '• Go to WordPress Admin → Users → Profile → Application Passwords',
      '• Generate a new Application Password specifically for this service',
      '• Copy the password exactly as shown (including spaces if any)',
      '• Ensure the username matches exactly (check case sensitivity)',
      '• Verify the user has Administrator role permissions'
    ];
  ...
  private async fixImageAltTextComprehensive(
    ...
    try {
      ...
      const [posts, pages] = await Promise.all([
        this.getWordPressContent(creds, 'posts').catch(e => {
          this.addLog(`Failed to fetch posts: ${e.message}`, 'warning');
          return [];
        }),
        this.getWordPressContent(creds, 'pages').catch(e => {
      ...
      for (const content of allContent.slice(0, 10)) { // Limit to first 10 items
        try {
          const $ = cheerio.load(content.content.rendered || content.content);
          const imagesWithoutAlt = $('img:not([alt]), img[alt=""]');
          
          if (imagesWithoutAlt.length > 0) {
            this.addLog(`Processing ${imagesWithoutAlt.length} images in "${content.title.rendered || content.title}"`);
            
            let contentModified = false;
            
            for (let i = 0; i < Math.min(imagesWithoutAlt.length, 5); i++) { // Limit to 5 images per post
              ...
              if (src && !src.startsWith('data:')) {
                ...
                const altText = await this.generateAltText(src, content.title.rendered || content.title);
                ...
                applied.push({
                  type: 'missing_alt_text',
                  description: `Added alt text to image in "${content.title.rendered || content.title}"`,
            ...
            if (contentModified) {
              try {
                ...
                this.addLog(`Updated content for: ${content.title.rendered || content.title}`, 'success');
              } catch (updateError) {
                const errorMsg = `Failed to update content: ${updateError instanceof Error ? updateError.message : 'Unknown error'}`;
          ...
          const errorMsg = `Error processing content "${content.title.rendered || content.title}": ${contentError instanceof Error ? contentError.message : 'Unknown error'}`;
          errors.push(errorMsg);
          this.addLog(errorMsg, 'warning');
        }
      }

      this.addLog(`Fixed alt text for ${totalImagesFixed} images total`, 'success');

    } catch (error) {
      const errorMsg = `Alt text fix failed: ${error instanceof Error ? error.message : 'Unknown error'}`;
      errors.push(errorMsg);
      this.addLog(errorMsg, 'error');
    ...
    try {
      ...
      for (const content of allContent) {
        try {
          ...
          const contentText = this.extractTextFromHTML(content.content?.rendered || content.content || '');
          ...
          if (!excerpt || excerpt.length < 120) {
            ...
            try {
              ...
              this.addLog(`Updated meta description for: ${content.title?.rendered || content.title}`, 'success');
            } catch (updateError) {
              const errorMsg = `Failed to update meta description: ${updateError instanceof Error ? updateError.message : 'Unknown error'}`;
              errors.push(errorMsg);
              applied.push({
                type: 'missing_meta_description',
          ...
          this.addLog(`Error processing meta description for "${content.title?.rendered || content.title}": ${contentError instanceof Error ? contentError.message : 'Unknown error'}`, 'warning');
        }
      }

    } catch (error) {
      const errorMsg = `Meta description fix failed: ${error instanceof Error ? error.message : 'Unknown error'}`;
      errors.push(errorMsg);
      this.addLog(errorMsg, 'error');
    }
        ...
        try {
          ...
          if (currentTitle.length > 60 || currentTitle.length < 30) {
            const optimizedTitle = await this.optimizeTitle(
              currentTitle,
              this.extractTextFromHTML(content.content?.rendered || content.content || '')
            );
            ...
            try {
              ...
              this.addLog(`Optimized title: "${currentTitle}" -> "${optimizedTitle}"`, 'success');
            } catch (updateError) {
              const errorMsg = `Failed to update title: ${updateError instanceof Error ? updateError.message : 'Unknown error'}`;
              errors.push(errorMsg);
              applied.push({
                type: 'poor_title_tag',
          ...
          this.addLog(`Error processing title for "${content.title?.rendered || content.title}": ${contentError instanceof Error ? contentError.message : 'Unknown error'}`, 'warning');
        }
      }

    } catch (error) {
      const errorMsg = `Title optimization failed: ${error instanceof Error ? error.message : 'Unknown error'}`;
      errors.push(errorMsg);
      this.addLog(errorMsg, 'error');
    }
        ...
        try {
          ...
          if (needsUpdate) {
            try {
              ...
              this.addLog(`Fixed heading structure for: ${content.title?.rendered || content.title}`, 'success');
            } catch (updateError) {
              const errorMsg = `Failed to fix heading structure: ${updateError instanceof Error ? updateError.message : 'Unknown error'}`;
              errors.push(errorMsg);
              applied.push({
                type: 'heading_structure',
          ...
          this.addLog(`Error processing headings for "${content.title?.rendered || content.title}": ${contentError instanceof Error ? contentError.message : 'Unknown error'}`, 'warning');
        }
      }

    } catch (error) {
      const errorMsg = `Heading structure fix failed: ${error instanceof Error ? error.message : 'Unknown error'}`;
      errors.push(errorMsg);
      this.addLog(errorMsg, 'error');
    }
  ...
  private async getWordPressContent(creds: WordPressCredentials, type: 'posts' | 'pages') {
    const endpoint = `${creds.url}/wp-json/wp/v2/${type}`;
  ...
  private async updateWordPressContent(creds: WordPressCredentials, id: number, data: any) {
    const endpoint = `${creds.url}/wp-json/wp/v2/posts/${id}`;
    const auth = Buffer.from(`${creds.username}:${creds.applicationPassword}`).toString('base64');
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to update content: ${response.status} ${response.statusText} - ${errorBody}`);
    }
  ...
  private async generateAltText(imageSrc: string, context: string): Promise<string> {
    ...
    try {
      const prompt = `Alt text for image "${imageSrc}" in context "${context}". Max 100 chars, descriptive, no quotes:`;
      
      const result = await this.callAIProvider(provider, 'You are an accessibility expert. Return only the alt text.', prompt, 50);
      return result.substring(0, 100).trim() || 'Descriptive image';
    } catch {
  ...
  private async generateMetaDescription(title: string, content: string): Promise<string> {
    ...
    try {
      const prompt = `Meta description for "${title}". Content: "${content.substring(0, 300)}". 120-160 chars, compelling, SEO-optimized:`;
      
      const result = await this.callAIProvider(provider, 'You are an SEO expert. Return only the meta description.', prompt, 100);
  ...
  private async optimizeTitle(currentTitle: string, content: string): Promise<string> {
    ...
    try {
      const prompt = `Optimize title "${currentTitle}" using content "${content.substring(0, 200)}". 30-60 chars, engaging, SEO-focused:`;
      
      const result = await this.callAIProvider(provider, 'You are an SEO expert. Return only the optimized title.', prompt, 50);
  ...
  private selectAIProvider(): string | null {
    return (process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY_ENV_VAR) ? 'openai' :
           (process.env.ANTHROPIC_API_KEY ? 'anthropic' : null);
  }

  private async callAIProvider(provider: string, systemMessage: string, userMessage: string, maxTokens: number = 500): Promise<string> {
    if (provider === 'openai') {
  ...
  private calculateEstimatedImpact(fixes: AIFix[]): string {
    const successful = fixes.filter(f => f.success);
    const highImpactFixes = successful.filter(f => f.impact === 'high').length;
    const mediumImpactFixes = successful.filter(f => f.impact === 'medium').length;
  ...
  private async createWebsiteBackup(website: any, userId: string): Promise<void> {
    ...
    try {
      ...
      this.addLog(`Backup creation failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'warning');
      // Don't throw - backup failure shouldn't stop the fix process
    }
  }

  // Get available fix types for a website
  async getAvailableFixTypes(websiteId: string, userId: string): Promise<{
    ...
    try {
      ...
      const analysis = await this.analyzeWebsiteForAllFixes(website.url, latestReport);
      
      const availableFixTypes = [...new Set(analysis.fixableIssues.map((fix: any) => fix.type))];
      
      // Count issues by type
      const breakdown = analysis.fixableIssues.reduce((acc: Record<string, number>, fix: any) => {
      ...
      this.addLog(`Failed to get available fix types: ${error instanceof Error ? error.message : 'Unknown error'}`, 'error');
      return {
        availableFixes: [],
        totalFixableIssues: 0,
        estimatedTime: '0 minutes',
