...
export class AutoScheduleService {
    ...
    try {
      ...
      for (const schedule of schedules) {
        ...
        if (schedule.costToday >= schedule.maxDailyCost) {
          console.log(`💰 Daily cost limit reached for schedule: ${schedule.name}`);
          continue;
        }

        // Check monthly post limit
        if (schedule.postsThisMonth >= schedule.maxMonthlyPosts) {
          console.log(`📊 Monthly post limit reached for schedule: ${schedule.name}`);
          continue;
        }
        ...
        try {
          ...
          if (generationResult.success) {
            ...
            if (schedule.autoPublish) {
              const publishTime = new Date(now.getTime() + schedule.publishDelay * 60 * 60 * 1000);
              ...
              console.log(`📅 Content scheduled for publishing at ${publishTime}`);
            }
      ...
      console.log(`✅ Auto-generation complete: ${generated} generated, ${failed} failed`);
  ...
  private shouldRunSchedule(schedule: any, currentHour: number, currentDay: string): boolean {
    ...
    switch (schedule.frequency) {
      ...
      case 'biweekly':
        ...
        const daysSinceLastRun = Math.floor((Date.now() - new Date(schedule.lastRun).getTime()) / (1000 * 60 * 60 * 24));
