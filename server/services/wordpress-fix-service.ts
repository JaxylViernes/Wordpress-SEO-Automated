...
export class WordPressFixService {
  ...
  async fixImageAltText(credentials: WordPressCredentials): Promise<{
    ...
    try {
      ...
      const imagesWithoutAlt = mediaItems.filter(
        (item: any) => !item.alt_text || item.alt_text.trim() === ""
      );
      ...
      for (const image of imagesWithoutAlt) {
        try {
          ...
          await this.makeWPRequest(credentials, `/media/${image.id}`, "PUT", {
            alt_text: altText,
          });

          console.log(
            `✅ Updated alt text for image ${image.id}: "${altText}"`
          );
          fixed++;
        } catch (error) {
          const errorMsg = `Failed to update image ${image.id}: ${
      ...
      for (const content of allContent) {
        try {
          ...
          if (!currentMeta || currentMeta.length < 120) {
            ...
            const updateData = await this.updateMetaDescription(
              credentials,
              content.id,
              metaDescription
            );
            ...
            console.log(
              `✅ Updated meta description for ${content.type} ${content.id}`
            );
            fixed++;
          }
        } catch (error) {
          const errorMsg = `Failed to update meta for ${content.type} ${
            content.id
          }: ${error instanceof Error ? error.message : "Unknown error"}`;
          console.error(errorMsg);
          errors.push(errorMsg);
      ...
      for (const content of allContent) {
        try {
          ...
          if (this.needsTitleOptimization(currentTitle)) {
            ...
            console.log(
              `✅ Optimized title for ${content.type} ${content.id}: "${optimizedTitle}"`
            );
            fixed++;
          }
        } catch (error) {
          const errorMsg = `Failed to update title for ${content.type} ${
            content.id
          }: ${error instanceof Error ? error.message : "Unknown error"}`;
          console.error(errorMsg);
          errors.push(errorMsg);
      ...
      for (const content of allContent) {
        try {
          ...
          if (fixedContent !== currentContent) {
            ...
            console.log(
              `✅ Fixed heading structure for ${content.type} ${content.id}`
            );
            fixed++;
          }
        } catch (error) {
          const errorMsg = `Failed to fix headings for ${content.type} ${
            content.id
          }: ${error instanceof Error ? error.message : "Unknown error"}`;
          console.error(errorMsg);
          errors.push(errorMsg);
  ...
  private async generateMetaDescription(content: any): Promise<string> {
    ...
    const contentText =
      content.content?.rendered?.replace(/<[^>]*>/g, "").substring(0, 300) ||
      "";
  ...
  private async updateMetaDescription(
    ...
    try {
      ...
      await this.makeWPRequest(credentials, `/yoast/v1/meta/${postId}`, "PUT", {
        meta_description: metaDescription,
      });
    } catch (yoastError) {
      // Fall back to custom meta field
      await this.makeWPRequest(credentials, `/posts/${postId}/meta`, "POST", {
  ...
  private async generateOptimizedTitle(content: any): Promise<string> {
    const currentTitle = content.title?.rendered || "";
    const contentText =
      content.content?.rendered?.replace(/<[^>]*>/g, "").substring(0, 200) ||
      "";
