...
export class AuthService {
  ...
  async createUser(userData: UserData): Promise<AuthUser> {
    try {
      console.log('🔍 Creating user:', { username: userData.username, hasEmail: !!userData.email });
      ...
      const existingUser = await storage.getUserByUsername(userData.username.toLowerCase().trim());
      if (existingUser) {
