...
export interface ContentGenerationRequest {
  ...
  //nadagdag - Add auto-publishing fields
...
//nadagdag - Add interface for extended result with publishing info
...
`;
  ...
  //nadagdag - Updated generateContent to return ContentGenerationResultWithPublishing
  async generateContent(
    ...
    try {
      ...
      //nadagdag - Log auto-publishing configuration if present
...
- Robotic transitions between paragraphs`;
      ...
      //nadagdag - Step 8: Save generated content and handle auto-publishing
      ...
      //nadagdag - Save the generated content first
      try {
        const contentData = {
          websiteId: request.websiteId,
          userId: request.userId,
        ...
        //nadagdag - Save content to database
        const savedContent = await storage.createGeneratedContent(contentData);
        contentId = savedContent.id;
        
        console.log(`✅ Content saved with ID: ${contentId}`);
        
        //nadagdag - Handle auto-publishing if configured
        if (request.isAutoGenerated && request.autoScheduleId && request.autoPublish) {
          console.log(`🚀 Processing auto-publishing for content ${contentId}...`);
          
          //nadagdag - Calculate scheduled date based on publishDelay
          if (request.publishDelay === 0) {
            //nadagdag - Immediate publishing
            scheduledDate = new Date();
            
            try {
              //nadagdag - Create content_schedule entry for immediate publishing
              await storage.createContentSchedule({
                contentId: contentId,
                websiteId: request.websiteId,
                userId: request.userId,
                scheduled_date: scheduledDate, // CRITICAL: Never null, set to NOW
                status: 'publishing',
                title: contentResult.title,
                metadata: {
                  autoGenerated: true,
              ...
              //nadagdag - Publish to WordPress immediately
              ...
              if (publishResult.success) {
                ...
                //nadagdag - Update content status to published
                ...
                //nadagdag - Update content_schedule status
                await storage.updateContentScheduleByContentId(contentId, {
                  status: 'published',
                  published_at: publishedAt
                });
                
                console.log(`✅ Content published immediately to WordPress (Post ID: ${publishResult.postId})`);
              } else {
                console.error(`❌ Failed to publish immediately: ${publishResult.error}`);
                
                //nadagdag - Update schedule status to failed
                await storage.updateContentScheduleByContentId(contentId, {
                  status: 'failed',
              ...
              //nadagdag - Still ensure schedule entry exists with error status
              try {
                await storage.createContentSchedule({
                  contentId: contentId,
                  websiteId: request.websiteId,
                  userId: request.userId,
                  scheduled_date: new Date(), // Set to NOW even on error
                  status: 'failed',
                  title: contentResult.title,
                  metadata: {
                    autoGenerated: true,
                ...
                console.error(`❌ Failed to create error schedule entry: ${scheduleError}`);
              }
            }
            ...
            try {
              //nadagdag - Create content_schedule entry for delayed publishing
              await storage.createContentSchedule({
                contentId: contentId,
                websiteId: request.websiteId,
                userId: request.userId,
                scheduled_date: scheduledDate, // CRITICAL: Set future date, never null
                status: 'scheduled',
                title: contentResult.title,
                metadata: {
                  autoGenerated: true,
          ...
          //nadagdag - Auto-generated but not auto-publishing (save as draft)
          try {
            await storage.createContentSchedule({
              contentId: contentId,
              websiteId: request.websiteId,
              userId: request.userId,
              scheduled_date: new Date(), // CRITICAL: Set to NOW for drafts, never null
              status: 'draft',
              title: contentResult.title,
              metadata: {
                autoGenerated: true,
        ...
        console.error(`❌ Failed to save content: ${saveError.message}`);
      ...
      const result: ContentGenerationResultWithPublishing = {
        ...
        aiProvider: request.aiProvider,
        qualityChecks,
        //nadagdag - Add new fields for auto-publishing
        contentId: contentId,
        published: published,
      ...
      console.log(
        ...
        }${published ? ' - PUBLISHED IMMEDIATELY' : scheduledForPublishing ? ' - SCHEDULED FOR PUBLISHING' : ''}`
      );
  ...
  //nadagdag - Helper method to publish content to WordPress
  private async publishToWordPress(
    ...
    try {
      ...
      // Call WordPress API to publish
      // This is a placeholder - implement your actual WordPress publishing logic
      const wpResponse = await this.callWordPressApi(website, content);
      
      if (wpResponse.success) {
        return { 
          success: true, 
          postId: wpResponse.postId 
        };
      } else {
        return { 
          success: false, 
          error: wpResponse.error || 'WordPress publishing failed' 
        };
      }
    } catch (error: any) {
  ...
  //nadagdag - Placeholder for WordPress API call
  private async callWordPressApi(website: any, content: any): Promise<any> {
    // TODO: Implement actual WordPress REST API call
    // This should use the website's WordPress URL and authentication
    // to create a new post with the generated content
