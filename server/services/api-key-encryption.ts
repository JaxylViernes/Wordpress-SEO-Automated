...
export class ApiKeyEncryptionService {
  ...
  private getEncryptionKey(): string {
    const key = process.env.API_KEY_ENCRYPTION_SECRET || process.env.SECRET_KEY || 'default-development-key-change-in-production';
    if (!key || key === 'default-development-key-change-in-production') {
      console.warn('⚠️ Using default encryption key. Set API_KEY_ENCRYPTION_SECRET in production!');
    }
    return key;
  }
