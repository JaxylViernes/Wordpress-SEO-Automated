...
class AIFixService {
  ...
  private validateRequiredMethods(): boolean {
    ...
    for (const [methodName, description] of Object.entries(requiredMethods)) {
      ...
      if (!method) {
        this.addLog(
          `❌ Missing method: ${methodName} - ${description}`,
          "error"
        );
        allValid = false;
  ...
  private verifyFixStrategies(): void {
    ...
    if (missingMethods.length > 0) {
      ...
      this.addLog(
        `Warning: ${missingMethods.length} fix strategies are not implemented`,
        "warning"
      );
    }
  }
  ...
  private listAvailableStrategies(): string[] {
    ...
    if (missing.length > 0) {
      this.addLog(
        `⚠️ Missing fix strategies: ${missing.join(", ")}`,
        "warning"
      );
    }

    this.addLog(
      `✅ Available fix strategies: ${existing.join(", ")}`,
      "success"
    );
  ...
  private getFixStrategy(
    ...
    this.addLog(
      `Looking for strategy: ${fixType} (normalized: ${normalizedType})`,
      "info"
    );
    ...
    this.addLog(
      `Found strategy method: ${methodName} for ${fixType}`,
      "success"
    );
  ...
  private async fixWordPressContent(
    ...
    try {
      ...
      this.addLog(
        `Processing mode: ${
          processingOptions?.mode || "SAMPLE"
        } (max: ${maxItems}, batch: ${batchSize})`,
        "info"
      );
      ...
      this.addLog(
        `Fetched ${allContent.length} content items to process`,
        "info"
      );
      ...
      for (let i = 0; i < allContent.length; i += batchSize) {
        const batch = allContent.slice(
          i,
          Math.min(i + batchSize, allContent.length)
        );
        ...
        for (const content of batch) {
          const originalImages = this.extractImages(
          ...
          if (processingOptions?.progressCallback) {
            processingOptions.progressCallback(
              processedCount,
              allContent.length
            );
          }
        ...
        if (i + batchSize < allContent.length) {
          this.addLog(
            `Waiting ${limits.delayBetweenBatches}ms before next batch...`,
            "info"
          );
      ...
      if (applied.length === 0 && errors.length === 0) {
        this.addLog(
          `No updates needed for ${fixes[0].type} - content already compliant`,
          "info"
        );
    ...
    for (const url of priorityUrls) {
      try {
        const slug = url
          ...
          .pop();
        if (!slug) continue;
        const pageEndpoint = `${creds.url.replace(
          /\/$/,
          ""
        )}/wp-json/wp/v2/pages?slug=${slug}`;
        const postEndpoint = `${creds.url.replace(
          /\/$/,
          ""
      ...
      async (content, fix) => {
        ...
        if (hasProperStructure && hasProperHierarchy) {
          return {
            ...
            data: {},
            description:
              "Heading structure already optimal (1 H1 tag with proper hierarchy)",
          };
        }
        ...
        if (h1s.length > 1) {
          h1s.each((index, el) => {
            if (index > 0) {
              ...
              changes.push(
                `Converted duplicate H1 "${h1Text.substring(0, 30)}..." to H2`
              );
              updated = true;
            }
          });
        ...
        if (h1s.length === 0) {
          const title =
            content.title?.rendered || content.title || "Page Title";
        ...
        headings.forEach((heading) => {
          ...
          if (currentLevel > previousLevel + 1 && previousLevel !== 0) {
            ...
            if (correctLevel <= 6) {
              ...
              changes.push(
                `Fixed heading hierarchy: H${currentLevel} → H${correctLevel} for "${headingText.substring(
                  0,
                  30
                )}..."`
              );
      ...
      async (content, fix) => {
        ...
        imagesWithoutAlt.each((_, img) => {
          ...
          if (src && !src.startsWith("data:")) {
            ...
            const altText = this.generateFallbackAltText(
              src,
        ...
        if (cleanExcerpt.length >= 120 && cleanExcerpt.length <= 160) {
          return {
            ...
            description: `Meta description already optimal (${
              cleanExcerpt.length
            } chars): "${cleanExcerpt.substring(0, 50)}..."`,
          };
        }
    ...
    return this.fixWordPressContent(
      ...
      async (content, fix) => {
        ...
        return {
          ...
          description: `Improved content quality (score: ${analysis.score} → ~${
            analysis.score + 15
          })`,
        };
      },
      ...
      async (content, fix) => {
        ...
        if (optimized === contentHtml) {
          return {
            updated: false,
            data: {},
            description: "Keywords already optimized",
          };
        }
...
Return ONLY the improved HTML content without ANY meta-commentary or explanations.`;
  ...
  private async updateWordPressContent(
    ...
    if (data.content) {
      ...
      const resultImageCount = (resultContent.match(/<img[^>]*>/gi) || [])
        .length;
      const resultCloudinaryCount = (resultContent.match(/cloudinary/gi) || [])
        .length;
  ...
  private cleanAIResponse(content: string): string {
    ...
    const htmlMatch = cleaned.match(
      /<(!DOCTYPE|html|body|div|p|h[1-6]|article|section)[\s>]/i
    );
  ...
  private async updateIssueStatusesAfterFix(
    ...
    try {
      const fixingIssues = await storage.getTrackedSeoIssues(
        websiteId,
        userId,
        {
          status: ["fixing"],
      ...
      for (const fix of fixes) {
        if (!fixToIssueMap.has(fix)) {
          ...
          const matchingIssue = fixingIssues.find((issue) => {
            ...
            return (
              issue.issueType === mappedType || this.isRelatedIssue(issue, fix)
            );
          });
      ...
      const unmatchedIssues = fixingIssues.filter(
        (issue) => !processedIssueIds.has(issue.id)
      );
  ...
  private calculateDetailedBreakdown(fixes: AIFix[]) {
    ...
    return {
      ...
      imagesOptimized: successful.filter((f) => f.type === "image_optimization")
        .length,
      contentQualityImproved: successful.filter(
  ...
  async getAvailableFixTypes(
    ...
    try {
      ...
      const availableFixTypes = [
        ...new Set(fixableIssues.map((fix) => fix.type)),
      ];
          ...
          for (const para of paragraphs) {
            ...
            for (const [id, data] of contentMap) {
              ...
              for (const keyword of data.keywords) {
                if (
                  paraText.toLowerCase().includes(keyword) &&
                  !$(para).html()?.includes("href")
                ) {
    ...
    return this.fixWordPressContent(
      ...
      async (content, fix) => {
          ...
          if (href && !href.includes(creds.url)) {
            ...
            if (changes.length > 0) {
              ...
              specificChanges.push(
                `Link "${linkText}...": added ${changes.join(", ")}`
              );
            }
          }
        });
        ...
        return {
          ...
          data: {},
          description:
            "All external links already have proper security attributes (rel='noopener noreferrer' and target='_blank')",
        };
      },
      userId
    );
  ...
  private async generateSrcSetSafe(
    src: string,
    creds?: WordPressCredentials
  ): Promise<string | null> {
    ...
    return this.fixWordPressContent(
      ...
      async (content, fix) => {
        ...
        return {
          ...
          data: {},
          description:
            "All images already optimized with lazy loading and dimensions",
        };
      },
      userId
    );
      ...
      async (content, fix) => {
        ...
        if (wordCount >= 500) {
          return {
            updated: false,
            data: {},
            description: "Content length already sufficient",
          };
        }
        ...
        try {
          ...
          return {
            ...
            description: `Expanded thin content from ${wordCount} to ~${
              wordCount * 2
            } words`,
          };
        } catch (error) {
...
class ContentOptimizer {
    ...
    if (paragraphs.length > 0 && keywords.length > 0) {
      ...
      if (currentDensity < 1 && paragraphs.length > 3) {
        ...
        if (!keywords.some((kw) => middleText.toLowerCase().includes(kw))) {
          const naturalAddition = this.addKeywordNaturally(
  ...
  private createNaturalIntro(keyword: string, existingText: string): string {
    const intros = [
      `When it comes to ${keyword}, ${existingText
        .charAt(0)
        .toLowerCase()}${existingText.slice(1)}`,
      `${existingText} This is especially true for ${keyword}.`,
      `Understanding ${keyword} starts with knowing that ${existingText
        .charAt(0)
        .toLowerCase()}${existingText.slice(1)}`,
    ];
