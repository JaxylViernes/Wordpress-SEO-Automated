...
export class ApiValidationService {
  ...
  async validateGooglePageSpeedKey(apiKey: string): Promise<{ valid: boolean; error?: string }> {
    try {
      ...
      if (response.ok) {
        ...
        return { valid: false, error: 'Google PageSpeed API configuration error' };
      } else if (response.status === 403) {
        return { valid: false, error: 'Google PageSpeed API key lacks required permissions' };
      } else if (response.status === 429) {
        return { valid: false, error: 'Google PageSpeed API quota exceeded' };
      } else {
        return { valid: false, error: `Google PageSpeed API error: ${response.status}` };
      }
    } catch (error) {
      return { 
        valid: false, 
        error: error instanceof Error ? error.message : 'Failed to validate Google PageSpeed key' 
      };
    }
  }
