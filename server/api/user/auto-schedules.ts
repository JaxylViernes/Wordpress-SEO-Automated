...
router.post("/:id/run", async (req, res) => {
  try {
    ...
    console.log('Manual run initiated for schedule:', {
      ...
      userId: userId,
      websiteId: schedule.websiteId,
      //nadagdag - Log publishing settings
    ...
    try {
      const contentRequest = {
        ...
        //nadagdag - CRITICAL: Pass publishing settings to AI service
      ...
      //nadagdag - Log publishing configuration
      ...
      console.log('Content generation result:', {
        ...
        //nadagdag - Log if content was published
        wasPublished: result?.published || false
      });
      
      if (result && result.contentId) {
        //nadagdag - Handle immediate publishing if configured
        if (schedule.autoPublish && schedule.publishDelay === 0) {
          ...
          try {
            ...
            if (!scheduleEntry) {
              //nadagdag - Create schedule entry if it doesn't exist (fallback)
          ...
          //nadagdag - Handle delayed publishing
          try {
          ...
          //nadagdag - Handle draft (no auto-publish)
          try {
            await storage.createContentSchedule({
              contentId: result.contentId,
              websiteId: schedule.websiteId,
        ...
        await storage.createActivityLog({
          ...
          metadata: {
            ...
            contentId: result.contentId,
            cost: totalCost,
            //nadagdag - Log publishing status
        ...
        res.json({ 
          ...
          topic,
          cost: totalCost,
          //nadagdag - Include publishing status in response
