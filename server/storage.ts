...
async clearAllSeoData(websiteId: string, userId: string): Promise<{
  ...
  try {
    ...
    // Get the latest report to preserve it
    ...
    if (latestReport) {
      // Delete all reports EXCEPT the latest one
      const deleteResult = await db
        ...
        .where(
          and(
            eq(seoReports.websiteId, websiteId),
            eq(seoReports.userId, userId),
            not(eq(seoReports.id, latestReport.id))  // Exclude latest report
          )
        );
      ...
      console.log('No SEO reports found to delete');
    }
    
    // For tracked issues, keep issues from the latest report if it exists
    let deletedIssuesCount = 0;
    
    if (latestReport?.createdAt) {
      // Delete issues that haven't been seen since the latest report
      const deleteIssuesResult = await db
        ...
        .where(
          and(
            eq(seoIssueTracking.websiteId, websiteId),
            eq(seoIssueTracking.userId, userId),
            // Only delete issues last seen before the latest report
      ...
      // No latest report, delete all issues
      const deleteIssuesResult = await db
        .delete(seoIssueTracking)
        .where(
          and(
